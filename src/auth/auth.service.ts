...
export class AuthService {
  ...
  login({ id, email }): AccessAndRefreshToken {
    return {
      appAccessToken: this.jwtService.sign(
        { email },
        {
          expiresIn: '12 hrs',
          subject: String(id),
      ...
      appRefreshToken: this.jwtService.sign(
        { email },
        {
          expiresIn: '30 days',
          subject: String(id),
  ...
  async kakaoAuth(tokens: AuthDto): Promise<AccessAndRefreshToken> {
    ...
    const kakaoUser: KakaoAccountUsed = await this.kakaoService.getKakaoUser(
      tokens.accessToken,
    );
    if (kakaoUser) {
      if (
        kakaoUser.email == undefined ||
        kakaoUser.profile_image_url == undefined ||
        kakaoUser.thumbnail_image_url == undefined
      ) {
      ...
      if (userAlreadyExist) {
        ...
        try {
          ...
          const newAsset = await queryRunner.manager
    ...
    try {
      ...
      if (userAlreadyExist) {
        ...
        try {
          newUser = await queryRunner.manager.getRepository(Users).save({
            nickname:
              !data.fullName.familyName && !data.fullName.givenName
                ? ''
                : data.fullName.familyName + data.fullName.givenName,
